...
int Example::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    char* a_cstr = (char*) this->a.c_str();
    tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &a_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int Example::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    int32_t __a_len__;
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &__a_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__a_len__ > maxlen - pos) return -1;
    this->a.assign(((const char*)buf) + offset + pos, __a_len__ - 1);
    pos += __a_len__;
...
{
    int enc_size = 0;
    enc_size += this->a.size() + 4 + 1;
    return enc_size;
}

uint64_t Example::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0x067398db165e69c2LL;
